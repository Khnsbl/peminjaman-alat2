...
<div class="max-w-2xl mx-auto">
    <div class="bg-white rounded-lg shadow p-6">
        <form method="POST" action="{{ route('admin.alat.store') }}" class="space-y-6">
            @csrf

            <div>
                <label for="kode_alat" class="block text-sm font-medium text-gray-700 mb-2">Kode Alat</label>
                <input type="text" id="kode_alat" name="kode_alat" value="{{ old('kode_alat') }}" required 
            ...
            <div>
                ...
                <input type="text" id="nama_alat" name="nama_alat" value="{{ old('nama_alat') }}" required 
            ...
            <div>
                ...
                <textarea id="deskripsi" name="deskripsi" rows="4" 
                    class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-indigo-500 @error('deskripsi') border-red-500 @enderror">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="text-red-600 text-sm mt-1">{{ $message }}</p>
                @enderror
            </div>
